...
func fetch_image(_token = ""):
	http_request.request_completed.connect(_on_image_received, CONNECT_ONE_SHOT)
	make_request("/api/image")

func _on_image_received(result, response_code, headers, body) -> ImageTexture:
	var image = Image.new()
